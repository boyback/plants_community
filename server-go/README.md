...
- **第三期**(+24 个):Market / Orders / Payments(Mock,含 webhook 业务串联)/ VIP 订阅+钻石兑换 / 拍卖(状态机+防狙击+保证金+违约处理)/ 地址簿 ✅
- **补充 (+5 个)**:求助帖(悬赏+采纳)/ 节日主题(13 个节日,自动投放+用户关闭) ✅

后续迭代:
- **第四期**:钻石流水 / 任务 / 任务 / 皮肤装扮 / 私信
...
| **VIP** | | | |
| `GET` | `/api/vip/plans` | | 5 档套餐(月/季/年/终身/钻石) |
| `POST` | `/api/vip/order` | ✅ | 创建现金 VIP 订单 |
| `POST` | `/api/vip/exchange` | ✅ | 钻石兑月卡(5000 钻石 = 30 天) |
