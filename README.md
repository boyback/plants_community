...
> 🆕 v0.3 新增:**交易区 / 钻石系统 / 任务中心 / 大会员订阅 / 等级权限 / 皮肤装扮**
...
>   - 10 级权限矩阵 + VIP 通道
> - **新玩法**:
>   - 求助帖 `type=help`:支持 `urgency` + `bountyPoints` 悬赏 + 作者采纳,被采纳者自动收到悬赏钻石
...
>   - VIP 套餐卡(月/季/年/终身/钻石兑换)按 locale 自动切语言
...
>   - `app/market/[id]/ProductDetailClient.tsx`(下单 modal 全量:数量/库存/地址/合计/返钻石)
...
>   - `components/market/ProductCard.tsx`(官方/肉友标签、返钻石、卖家名、已售罄)
...
>   - `/points` 钻石中心(6 个 tab + 14 种流水类型 + 余额卡全部 i18n)
...
| `/points` | 钻石中心(余额 + 皮肤商城 4 类 + 我的装扮 + 流水) |
| `/tasks` | 任务中心中心(每日/月度/成就任务 + 阶梯奖励 + 月度排行榜) |
| `/vip` | 大会员开通(月卡 / 季卡 / 年卡 / 终身 / 钻石兑月卡) |
...
| `/auth/me` | GET | 当前用户(含 EXP/钻石/VIP/装扮) |
...
| `/points/me` | GET | 我的钻石汇总 |
| `/points/ledger` | GET | 钻石流水 |
...
- 股东标识 + 金色变色昵称 + 专属皇冠头像框
- 专属评论气泡 / 钻石点赞 / 尊享表情包
- **10x 任务 / 钻石 / 经验加成**(发帖原本 +20,VIP 直接 +200)
...
- **保证金退还**:Demo 中以等值钻石(¥1 = 100 钻石)退给未胜出者(真实生产可走原支付渠道)
