...
export const GET = handler(async (req) => {
  ...
  const [exists, rows] = await Promise.all([
    ...
    prisma.$queryRaw<Array<{ total: bigint }>>`
      SELECT COUNT(*) AS total FROM species_compares
      WHERE userId = ${me.id}
    `,
  ]);
  return { compared: exists.length > 0, total: Number(rows[0]?.total ?? 0) };
});
...
export const POST = handler(async (req) => {
  ...
  if (existing.length > 0) {
    ...
    if (Number(rows[0]?.total ?? 0) >= 4) {
      return fail(400, '最多只能同时对比 4 个品种');
    }
    await prisma.$executeRaw`
      INSERT INTO species_compares (userId, speciesId, createdAt)
      VALUES (${me.id}, ${speciesId}, NOW())
    `;
    await incrementSpeciesDailyStat(speciesId, 'compares');
  }
  ...
  return { compared: existing.length === 0, total: Number(totalRows[0]?.total ?? 0) };
});
